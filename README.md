...
│   └── stock_data_common.py          # D/E共用模块 (数据获取/股票池)
│
├── docs/                              # 详细策略文档
│   ├── strategy_b.md
│   ├── strategy_c.md
...
- **策略B Vol缩放已修复**：v2.2修复了回测和信号生成中的vol scaling计算，现在使用每期实际权重的组合收益计算波动率
...
- **策略D/E 公共模块已抽取**：v2.2将共用代码移至`stock_data_common.py`
- **策略B 累计交易成本偏高**：11年累计18.02%（年化~1.6%）
...
| 日期 | 版本 | 更新内容 |
|------|------|---------|
| 2026-03-22 | v2.2 | 修复B vol scaling，抽取D/E公共模块stock_data_common.py，重跑全部回测验证 |
